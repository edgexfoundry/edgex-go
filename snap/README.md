...
this repository, three Java-based services, support-rulesengine and support-
scheduler, and device-virtual, as well as Vault, Kong, Cassandra, and the two go-based 
security micro services. The snap also contains a OpenJDK JRE used to run 
...
$ sudo snap connect edgexfoundry:daemon-notify
```
...
Note that you cannot install a local snap without using `--devmode` because some of the services require using `systemd-notify` to startup, which in turn requires the `daemon-notify` interface. If this interface is not connected (or the snap is not in devmode), then the services will fail to start and the snap's installation will be aborted. This is not a problem for the snap as downloaded/installed from the store because the assertions to auto-connect the `daemon-notify` interface are included when you download the snap from the store, but no such assertions exist for the locally developed snap because it hasn't been published to the snap store yet.