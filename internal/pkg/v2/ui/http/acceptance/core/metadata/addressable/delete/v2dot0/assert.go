...
// assertValid validates addressable delete response.
func assertValid(
	t *testing.T,
	router *mux.Router,
	...
	for _, id := range deletedIDs {
		errorResponse := v2dot0.GetNonExistentAddressableById(t, router, id)
		assert.Equal(t, infrastructure.StatusPersistenceNotFound, errorResponse.StatusCode)
	}
}

// assertV2dot0UseCaseValid validates addressable add response; we can't predict what will be
// returned so we consider valid response to have all non-zero fields.
func assertV2dot0UseCaseValid(
	t *testing.T,
	router *mux.Router,
	actual []byte,
	requestIDs []string,
	...
	if err == nil {
		assertValid(t, router, &responseDTO, requestIDs, deletedIDs)
		return
	}
	...
	if err == nil {
		for i := range responseDTOs {
			assertValid(t, router, &responseDTOs[i], requestIDs, deletedIDs)
		}
		return
	}
...
func assertV2dot0BatchWithOneValidAndOneError(
	...
	for i := range responseDTOs {
		...
		validExists = true
		assertValid(t, router, &responseDTO, []string{validRequestID}, deletedIDs)
	}
