...
// assertUpdated asserts the addressable in a readResponse equals the addressable in an updateRequest.
func assertUpdated(t *testing.T, updateRequest *dtoV2dot0.Request, readResponse *dtoReadV2dot0.Response) {
	assert.Equal(t, updateRequest.ID, readResponse.ID)
	assert.Equal(t, updateRequest.Name, readResponse.Name)
	assert.Equal(t, updateRequest.Protocol, readResponse.Protocol)
	assert.Equal(t, updateRequest.Method, readResponse.Method)
	assert.Equal(t, updateRequest.Address, readResponse.Address)
	assert.Equal(t, updateRequest.Port, readResponse.Port)
	assert.Equal(t, updateRequest.Path, readResponse.Path)
	assert.Equal(t, updateRequest.Publisher, readResponse.Publisher)
	assert.Equal(t, updateRequest.User, readResponse.User)
	assert.Equal(t, updateRequest.Password, readResponse.Password)
	assert.Equal(t, updateRequest.Topic, readResponse.Topic)
}

// assertValid validates addressable update response.
func assertValid(
	t *testing.T,
	router *mux.Router,
	response *dtoV2dot0.Response,
	requestIDs []string,
	updateRequests map[string]*dtoV2dot0.Request) {
	...
	assertUpdated(t, updateRequests[response.RequestID], v2dot0.GetAddressableById(t, router, &response.ID))
}
...
func assertV2dot0UseCaseOneValid(
	...
	if err == nil {
		assertValid(t, router, &responseDTO, requestIDs, updateRequests)
		return
	}
	...
	if err == nil {
		for i := range responseDTOs {
			assertValid(t, router, &responseDTOs[i], requestIDs, updateRequests)
		}
		return
	}
...
// assertV2dot0UseCaseWithOneValidAndOneError validates one successful result and one error result of a specific type.
func assertV2dot0UseCaseWithOneValidAndOneError(
	...
	for i := range responseDTOs {
		...
		validExists = true
		assertValid(t, router, &responseDTO, []string{validRequestID}, updateRequests)
	}
