...
func deviceStoredKey(id string) string {
	return fmt.Sprintf("%s%s%s", DeviceCollection, DBKeySeparator, id)
}
...
func addDevice(conn redis.Conn, d models.Device) (models.Device, errors.EdgeX) {
	...
	_ = conn.Send(ZADD, fmt.Sprintf("%s%s%s", DeviceCollectionServiceName, DBKeySeparator, d.ServiceName), d.Modified, storedKey)
	for _, label := range d.Labels {
		_ = conn.Send(ZADD, fmt.Sprintf("%s%s%s", DeviceCollectionLabel, DBKeySeparator, label), d.Modified, storedKey)
	}
	_, err = conn.Do(EXEC)
	if err != nil {
...
func deleteDevice(conn redis.Conn, device models.Device) errors.EdgeX {
	...
	_ = conn.Send(ZREM, fmt.Sprintf("%s%s%s", DeviceCollectionServiceName, DBKeySeparator, device.ServiceName), storedKey)
	for _, label := range device.Labels {
		_ = conn.Send(ZREM, fmt.Sprintf("%s%s%s", DeviceCollectionLabel, DBKeySeparator, label), storedKey)
	}
	_, err := conn.Do(EXEC)
	if err != nil {
...
func devicesByServiceName(conn redis.Conn, offset int, limit int, name string) (devices []models.Device, edgeXerr errors.EdgeX) {
	...
	objects, err := getObjectsByRevRange(conn, fmt.Sprintf("%s%s%s", DeviceCollectionServiceName, DBKeySeparator, name), offset, end)
	if err != nil {
