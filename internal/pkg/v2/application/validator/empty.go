...
// NotEmpty encapsulates the validation behavior for ensuring a string property is not empty.
func NotEmpty(
	requestID string,
	value string,
	errorStatus infrastructure.Status,
	r interface{}) (response interface{}, status infrastructure.Status) {

	if len(value) == 0 {
		return dtoError.NewResponse(dtoBase.NewResponse(requestID, r, errorStatus)), errorStatus
	}
