...
func sqlQueryAllEventAndDescWithCondsAndPage(descCol string, columns ...string) string {
	...
	return fmt.Sprintf(
		...
		whereCondition, descCol, columnCount+1, columnCount+2) // count WHERE conditions, then plus 1 and 2 as OFFSET, LIMIT sql arg
}
...
func sqlQueryAllReadingAndDescWithCondsAndPag(descCol string, columns ...string) string {
	...
	return fmt.Sprintf(
		...
		whereCondition, descCol, columnCount+1, columnCount+2) // count WHERE conditions, then plus 1 and 2 as OFFSET, LIMIT sql argument
}
...
func sqlQueryAllEventAndDescWithCondsAndPagAndUpperLimitTime(descCol string, upperLimitTimeRangeCol string, columns ...string) string {
	...
	return fmt.Sprintf(
		"SELECT %s FROM %s join %s on event.device_info_id = device_info.id WHERE %s ORDER BY %s DESC OFFSET $%d LIMIT $%d",
		eventColumns, eventTableName, deviceInfoTableName,
		whereCondition, descCol, columnCount+2, columnCount+3) //  the UpperLimitTime is the first sql argument, the OFFSET, LIMIT are second and third sql argument
}
...
func sqlQueryAllEventWithPaginationAndTimeRangeDescByCol(timeRangeCol string, descCol string, arrayColNames []string, columns ...string) string {
	...
	return fmt.Sprintf(
		"SELECT %s FROM %s join %s on event.device_info_id = device_info.id WHERE %s ORDER BY %s DESC OFFSET $%d LIMIT $%d",
		eventColumns, eventTableName, deviceInfoTableName,
		whereCondition, descCol, columnCount+3, columnCount+4) // there are two timeRange columns that need two sql argument, the OFFSET, LIMIT are third and fourth sql argument
}
...
func sqlQueryAllByColWithPaginationAndTimeRange(table string, columns ...string) string {
	...
	return fmt.Sprintf(
		...
		columnCount+3, columnCount+4) // there are two timeRange columns that need two sql argument, the OFFSET, LIMIT are third and fourth sql argument
}
