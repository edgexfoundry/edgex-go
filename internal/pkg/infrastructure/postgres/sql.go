...
func sqlQueryAllReadingAndDescWithCondsAndPag(descCol string, columns ...string) string {
	...
	return fmt.Sprintf("SELECT %s FROM %s join %s on reading.device_info_id = device_info.id WHERE %s ORDER BY %s DESC OFFSET $%d LIMIT $%d", readingColumns, readingTableName, deviceInfoTableName, whereCondition, descCol, columnCount+1, columnCount+2)
}
...
func sqlQueryAllEventAndDescWithCondsAndPagAndUpperLimitTime(descCol string, upperLimitTimeRangeCol string, columns ...string) string {
	...
	return fmt.Sprintf("SELECT %s FROM %s join %s on event.device_info_id = device_info.id WHERE %s ORDER BY %s DESC OFFSET $%d LIMIT $%d", eventColumns, eventTableName, deviceInfoTableName, whereCondition, descCol, columnCount+2, columnCount+3)
}
...
func sqlQueryAllEventWithPaginationAndTimeRangeDescByCol(timeRangeCol string, descCol string, arrayColNames []string, columns ...string) string {
	...
	return fmt.Sprintf("SELECT %s FROM %s join %s on event.device_info_id = device_info.id WHERE %s ORDER BY %s DESC OFFSET $%d LIMIT $%d",
		eventColumns, eventTableName, deviceInfoTableName, whereCondition, descCol, columnCount+3, columnCount+4)
}
...
func sqlQueryAllReadingWithPaginationAndTimeRangeDescByCol(timeRangeCol string, descCol string, arrayColNames []string, columns ...string) string {
	...
	return fmt.Sprintf("SELECT %s FROM %s join %s on reading.device_info_id = device_info.id WHERE %s ORDER BY %s DESC OFFSET $%d LIMIT $%d",
		readingColumns, readingTableName, deviceInfoTableName, whereCondition, descCol, columnCount+3, columnCount+4)
}
