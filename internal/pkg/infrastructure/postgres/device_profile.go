...
func (c *Client) AllDeviceProfiles(offset int, limit int, labels []string) (profiles []model.DeviceProfile, err errors.EdgeX) {
	...
	if len(labels) > 0 {
		...
		profiles, err = queryDeviceProfiles(ctx, c.ConnPool, sqlQueryContentByJSONFieldAndNamedArgWithPagination(deviceProfileTableName),
		...
		profiles, err = queryDeviceProfiles(ctx, c.ConnPool, sqlQueryContentWithNamedArgAndPagination(deviceProfileTableName),
...
func (c *Client) DeviceProfilesByModel(offset int, limit int, model string) ([]model.DeviceProfile, errors.EdgeX) {
	...
	return queryDeviceProfiles(ctx, c.ConnPool, sqlQueryContentByJSONFieldAndNamedArgWithPagination(deviceProfileTableName),
		pgx.NamedArgs{jsonContentCondition: queryObj, offsetCondition: offset, limitCondition: validLimit})
}
