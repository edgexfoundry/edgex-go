...
func (c *Client) DevicesByServiceName(offset int, limit int, name string) ([]model.Device, errors.EdgeX) {
	...
	return queryDevices(ctx, c.ConnPool, sqlQueryContentByJSONFieldAndNamedArgWithPagination(deviceTableName),
...
func (c *Client) AllDevices(offset int, limit int, labels []string) (devices []model.Device, err errors.EdgeX) {
	...
	if len(labels) > 0 {
		...
		devices, err = queryDevices(ctx, c.ConnPool, sqlQueryContentByJSONFieldAndNamedArgWithPagination(deviceTableName),
			pgx.NamedArgs{jsonContentCondition: queryObj, offsetCondition: offset, limitCondition: validLimit})
		if err != nil {
			return devices, errors.NewCommonEdgeX(errors.Kind(err), "failed to query all devices by labels", err)
		}
	} else {
		devices, err = queryDevices(ctx, c.ConnPool, sqlQueryContentWithNamedArgAndPagination(deviceTableName),
