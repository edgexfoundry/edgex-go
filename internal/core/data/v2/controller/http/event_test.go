...
func TestAllEventsByDeviceName(t *testing.T) {
	...
	dic := mocks.NewMockDIC()
	dbClientMock := &dbMock.DBClient{}
	dbClientMock.On("AllEventsByDeviceName", 0, 5, testDeviceA).Return([]models.Event{events[0], events[1]}, nil)
	dbClientMock.On("AllEventsByDeviceName", 1, 1, testDeviceA).Return([]models.Event{events[1]}, nil)
	dbClientMock.On("AllEventsByDeviceName", 4, 1, testDeviceB).Return([]models.Event{}, errors.NewCommonEdgeX(errors.KindEntityDoesNotExist, "query objects bounds out of range.", nil))
	dic.Update(di.ServiceConstructorMap{
		v2DataContainer.DBClientInterfaceName: func(get di.Get) interface{} {
			return dbClientMock
		},
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			req, err := http.NewRequest(http.MethodGet, v2.ApiAllEventByDeviceNameRoute, http.NoBody)
			query := req.URL.Query()
			...
			// Act
			recorder := httptest.NewRecorder()
			handler := http.HandlerFunc(ec.AllEventsByDeviceName)
			handler.ServeHTTP(recorder, req)
