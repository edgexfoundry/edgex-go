...
func AsyncPurgeEvent(ctx context.Context, dic *di.Container) errors.EdgeX {
	...
	deviceStore := container.DeviceStoreFrom(dic.Get)
	if deviceStore == nil {
		return errors.NewCommonEdgeX(errors.KindServerError, "nil deviceStore returned", nil)
	}
...
func purgeEvent(deviceName string, autoEvent models.AutoEvent, dic *di.Container) errors.EdgeX {
	config := container.ConfigurationFrom(dic.Get)
	if autoEvent.Retention == nil {
		// apply the default retention policy
		autoEvent.Retention = &models.Retention{
			MaxCap:   config.Retention.DefaultMaxCap,
			MinCap:   config.Retention.DefaultMinCap,
			Duration: config.Retention.DefaultDuration,
		}
	}
