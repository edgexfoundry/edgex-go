...
func TestExecuteGETCommandByDeviceIdAndCommandId(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := commandByDeviceID(tt.deviceId, TestCommandId, "", "", false, context.Background())
			if !reflect.DeepEqual(tt.errorValue, err) {
				t.Errorf("error type mismatch -- expected %v got %v", tt.errorValue, err)
				return
			}
		})
	}
