...
The Device Service will automatically register a device named `Random-Integer-Generator01` to EdgeX, which will start sending its random number readings into EdgeX itself.
...
Register the application as an export client::
...
When our Random Number Device Service registered the device `Random-Integer-Generator01`, ts used a `profile <https://github.com/edgexfoundry/device-random/blob/master/cmd/res/device.random.yaml>`_ which defines commands for changing the minimum and maximum values for the random numbers it will generate.
